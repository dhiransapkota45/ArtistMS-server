...
export class MusicController {
    ...
    public async updateMusic(req: Request, res: Response) {
        const musicExist = await this.musicService.getMusicById(Number(req.params.id));
        if (!musicExist) {
            return ApiResponse.error(res, 404, "Music not found");
        }

        const music = await this.musicService.updateMusicById(Number(req.params.id), req.body);
    ...
    public async deleteMusic(req: Request, res: Response) {
        ...
        const music = await this.musicService.deleteMusicById(Number(req.params.id));
    ...
    public async getMusicByArtistId(req: Request, res: Response) {
        const music = await this.musicService.getMusicByArtistId(Number(req.params.artist_id));
        return ApiResponse.success(res, "Music retrieved successfully", music, 200);
    }
}